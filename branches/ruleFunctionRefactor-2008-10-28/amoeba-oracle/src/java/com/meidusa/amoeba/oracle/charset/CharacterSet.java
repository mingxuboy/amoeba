...
public abstract class CharacterSet {
    ...
    public String toString(byte abyte0[], int i, int j) throws SQLException {
        String s = toStringWithReplacement(abyte0, i, j);
        byte abyte1[] = convert(s);
        if (j != abyte1.length) failCharacterConversion(this);
        for (int k = 0; k < j; k++)
            if (abyte1[k] != abyte0[i + k]) failCharacterConversion(this);
    ...
    public abstract byte[] convert(CharacterSet characterset, byte abyte0[], int i, int j) throws SQLException;

    public byte[] convertUnshared(CharacterSet characterset, byte abyte0[], int i, int j) throws SQLException {
    ...
    abstract int decode(CharacterWalker characterwalker) throws SQLException;

    abstract void encode(CharacterBuffer characterbuffer, int i) throws SQLException;

    static final void failCharacterConversion(CharacterSet characterset) throws SQLException {
        // DatabaseError.throwSqlException(55, characterset);
        throw new RuntimeException();
    }
    ...
    public static final char[] UTFToJavaChar(byte abyte0[], int i, int j) throws SQLException {
        ...
        label0: do {
            ...
            switch (i2 / 16) {
                ...
                case 14: // '\016'
                    ...
                    if (i2 != 244 && abyte0[i1 - 2] != -65 && abyte0[i1 - 1] != -67) characterconverterbehavior.onFailConversion(c2);
                    if (isHiSurrogate(c2)) {
                        if (k1 > l1 - 2) {
                            ai[0] = (j1 - i1) + 4;
                            break label0;
    ...
    public static final char[] AL32UTF8ToJavaChar(byte abyte0[], int i, int j, boolean flag) throws SQLException {
        Object obj = null;
        char ac1[];
        try {
            char ac[] = new char[j];
    ...
    public static final int convertAL32UTF8BytesToJavaChars(byte abyte0[], int i, char ac[], int j, int ai[],
                                                            boolean flag) throws SQLException {
        return convertAL32UTF8BytesToJavaChars(abyte0, i, ac, j, ai, flag, ac.length - j);
    }

    public static final int convertAL32UTF8BytesToJavaChars(byte abyte0[], int i, char ac[], int j, int ai[],
                                                            boolean flag, int k) throws SQLException {
        CharacterConverterBehavior characterconverterbehavior = flag ? CharacterConverterBehavior.REPLACEMENT : CharacterConverterBehavior.REPORT_ERROR;
        int l = ai[0];
        ai[0] = 0;
        int i1 = i;
    ...
    public static final int convertAL16UTF16BytesToJavaChars(byte abyte0[], int i, char ac[], int j, int k, boolean flag)
        ...
        for (int j1 = i + k; i1 + 1 < j1; i1 += 2) {
            ...
            if (isHiSurrogate(c)) {
                if ((i1 += 2) + 1 >= j1) continue;
                char c1 = (char) ((abyte0[i1] << 8) + (abyte0[i1 + 1] & 0xff));
                if (isLoSurrogate(c1)) ac[l++] = c;
                else ac[l++] = '\uFFFD';
                ac[l++] = c1;
            } else {
                ac[l++] = c;
    ...
    public static final int convertAL16UTF16LEBytesToJavaChars(byte abyte0[], int i, char ac[], int j, int k,
                                                               boolean flag) throws SQLException {
        CharacterConverterBehavior characterconverterbehavior = flag ? CharacterConverterBehavior.REPLACEMENT : CharacterConverterBehavior.REPORT_ERROR;
        int l = j;
        int i1 = i;
    ...
    public static final int convertJavaCharsToISOLATIN1Bytes(char ac[], int i, byte abyte0[], int j, int k)
        ...
        for (int l = 0; l < k; l++) {
            char c = ac[i + l];
            if (c > '\377') abyte0[j + l] = -65;
            else abyte0[j + l] = (byte) c;
        }
    ...
    static final char conv2ByteUTFtoUTF16(byte byte0, byte byte1) {
        if (byte0 < -62 || byte0 > -33 || !check80toBF(byte1)) return '\uFFFD';
        else return (char) ((byte0 & 0x1f) << 6 | byte1 & 0x3f);
    }

    static final char conv3ByteUTFtoUTF16(byte byte0, byte byte1, byte byte2) {
        if ((byte0 != -32 || !checkA0toBF(byte1) || !check80toBF(byte2)) && (byte0 < -31 || byte0 > -17 || !check80toBF(byte1) || !check80toBF(byte2))) return '\uFFFD';
        else return (char) ((byte0 & 0xf) << 12 | (byte1 & 0x3f) << 6 | byte2 & 0x3f);
    }

    static final char conv3ByteAL32UTF8toUTF16(byte byte0, byte byte1, byte byte2) {
        if ((byte0 != -32 || !checkA0toBF(byte1) || !check80toBF(byte2)) && (byte0 < -31 || byte0 > -20 || !check80toBF(byte1) || !check80toBF(byte2)) && (byte0 != -19 || !check80to9F(byte1) || !check80toBF(byte2)) && (byte0 < -18 || byte0 > -17 || !check80toBF(byte1) || !check80toBF(byte2))) return '\uFFFD';
        else return (char) ((byte0 & 0xf) << 12 | (byte1 & 0x3f) << 6 | byte2 & 0x3f);
    }
    ...
    public int encodedByteLength(String s) {
        ...
        else return convertWithReplacement(s).length;
    }

    public int encodedByteLength(char ac[]) {
        if (ac == null || ac.length == 0) return 0;
        else return convertWithReplacement(new String(ac)).length;
    }
