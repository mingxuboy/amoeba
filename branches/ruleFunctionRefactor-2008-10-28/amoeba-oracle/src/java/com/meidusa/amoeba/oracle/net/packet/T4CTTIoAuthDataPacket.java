...
public class T4CTTIoAuthDataPacket extends T4CTTIfunPacket implements T4CTTIoAuth {
    ...
    protected void marshal(AbstractPacketBuffer buffer) {
         ...
         * byte abyte2[] = o3loginclienthelper.getSessionKey(userStr, passwordStr, encryptedSK); byte abyte0[] =
         * meg.getConversion().StringToCharBytes(passwordStr); byte byte0; if (abyte0.length % 8 > 0) byte0 = (byte) (8
         * - abyte0.length % 8); else byte0 = 0; byte abyte1[] = new byte[abyte0.length + byte0];
         * System.arraycopy(abyte0, 0, abyte1, 0, abyte0.length); byte abyte3[] = o3loginclienthelper.getEPasswd(abyte2,
         * abyte1); byte[] password = new byte[2 abyte1.length + 1]; if (password.length < 2 abyte3.length)
         * DatabaseError.throwSqlException(413); RepConversion.bArray2Nibbles(abyte3, password);
         * password[password.length - 1] = RepConversion.nibbleToHex(byte0);
         */
