...
public class BlockedPacketIO {
	...
	public static final int readFully(InputStream in, byte[] b, int off, int len)
			throws IOException {
		if (len < 0) {
    ...
    public static final MysqlPacketBuffer readFullyPacketBuffer(InputStream mysqlInput) throws IOException {
            ...
            //数据包长度
            int packetLength = (packetHeaderBuf[0] & 0xff) +
                ((packetHeaderBuf[1] & 0xff) << 8) +
                ((packetHeaderBuf[2] & 0xff) << 16);
            ...
            int numBytesRead = readFully(mysqlInput, buffer, 4,
                    packetLength);
