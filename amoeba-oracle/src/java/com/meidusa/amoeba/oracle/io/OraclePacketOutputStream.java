...
public class OraclePacketOutputStream extends PacketOutputStream implements OraclePacketConstant {
    ...
    /**
     * @param packetwrittenWithHead 写数据的时候是否写入包头信息，true--表示写入数据的时候已经包括了包头信息， 否则则需要在调用{@link #returnPacketBuffer()}
     * 的时候需要实时生成包头信息
     */
    ...
    public ByteBuffer returnPacketBuffer() {
        ...
        if (!packetwrittenWithHead) {
             ...
             */
            /*
             * int count = _buffer.limit()-HEADER_PAD.length; _buffer.put((byte)(count & 0xff)); _buffer.put((byte)
             * (count >>> 8)); _buffer.put((byte) (count >>> 16));
             */
